...
def verifyScreenshot(df):
    ...
    for index, row in df.iterrows():
        if row["Compare_result"]=="Approved":
            ...
            scrnshtres = 'Verify'
            #reason = "Mismatch in actual result."

        chk_scrnsht.append(scrnshtres)
        #lstReason.append(reason)
        #print("fff")

    #converts list to series because dataframe column sud be of type series.
    res = pd.Series(chk_scrnsht)
    #lstres = pd.Series(lstReason)

    #adding columns from series
    df["Final Status"] = res.values
    #df["Reason"] = lstres.values
...
def generateSummary_SS(df):
    cnts = pd.DataFrame(df.TEST_ID.value_counts().reset_index())
    cnts.columns = ["TEST_ID", "Total Steps"]
    df1 = df.loc[df['Final Status'] == "Approved"]
    appcnts = pd.DataFrame(df1.TEST_ID.value_counts().reset_index())
    appcnts.columns = ["TEST_ID", "Approved"]
    df2 = df.loc[df['Final Status'] == "Verify"]
    vrfycnts = pd.DataFrame(df2.TEST_ID.value_counts().reset_index())
    vrfycnts.columns = ["TEST_ID", "Verify"]
    xxx = generateStepList(df2[['TEST_ID','test_step_name']])
    df10 = cnts.merge(appcnts, on='TEST_ID', how='left').merge(vrfycnts, on='TEST_ID', how='left').merge(xxx, on='TEST_ID', how='left')
...
def parse_contents(contents, filename, date):
    ...
    try:
        ...
        #calling all processing functions
        ...
        summaryStats_df= generateSummary_SS(df)
        ...
        print(e)
        return html.Div([
            filename + ': There was an error processing this file.'
